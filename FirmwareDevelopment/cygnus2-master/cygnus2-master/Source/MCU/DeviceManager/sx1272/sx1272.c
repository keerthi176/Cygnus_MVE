...
DioIrqHandler *DioIrq[] = { SX1272OnDio0Irq, SX1272OnDio1Irq,
                            ...
                            SX1272OnDio4Irq, SX1272OnDio5Irq, NULL };
...
// Note, the function parameter "uint32_t timeout" has been removed from function SX1272SetRx, and is set to const 0 in the code.
// The timeout is instead set by passing it to the function Radio.SetRxConfig().
...
int32_t SX1272ReadFrequencyDeviation( RadioModems_t modem )
{
   int32_t frequency_deviation;
   
   switch( modem )
   {
   case MODEM_FSK:
     frequency_deviation = (SX1272Read( REG_FEIMSB ) << 8) + SX1272Read( REG_FEILSB );
     break;
   case MODEM_LORA:
   {
      int32_t fdMSB = SX1272Read( REG_LR_FEIMSB );
      //The freq deviation is 20 bit signed.  Preserve the sign in 32 bit result
      if ( (fdMSB & 0x08) )
      {
         fdMSB |= 0xFFFFFFF0;
      }
      int32_t fdMID = (int32_t)SX1272Read( REG_LR_FEIMID );
      int32_t fdLSB = (int32_t)SX1272Read( REG_LR_FEILSB );
      int64_t frequency_error;
      frequency_error = ((int64_t)fdMSB << 16);
      frequency_error |= (fdMID << 8);
      frequency_error |= fdLSB;
      frequency_error = (frequency_error << 24) / 32000000;
      frequency_deviation = (int32_t)frequency_error;
   }
     break;
   default:
...
{
    // Set RESET pin to 1
    GpioInit( &SX1272.Reset, RADIO_RESET, PIN_OUTPUT_E, PIN_PUSH_PULL_E, PIN_NO_PULL_E, 1 );

    // Wait 1 ms
    DelayMs( 1 );

    // Configure RESET as input
    GpioInit( &SX1272.Reset, RADIO_RESET, PIN_INPUT_E, PIN_PUSH_PULL_E, PIN_NO_PULL_E, 1 );
...
void SX1272SetOpMode( uint8_t opMode )
{
   ...
   else if( opMode != RF_OPMODE_SLEEP )
   {
      SX1272SetAntSw( 0 );
   }
...
void SX1272OnTimeoutIrq( void )
{
    ...
    case RF_RX_RUNNING:
        ...
        break;
    case RF_TX_RUNNING:
       CO_PRINT_B_0(DBG_ERROR_E,"RF_TX_RUNNING Timeout\r\n");
        SX1272.Settings.State = RF_IDLE;
    ...
    switch( SX1272.Settings.State )
    {
        case RF_RX_RUNNING:
            ...
            case MODEM_FSK:
                ...
                {
                    RadioEvents->RxDone( RxTxBuffer, SX1272.Settings.FskPacketHandler.Size, SX1272.Settings.FskPacketHandler.RssiValue, 0, SX1272.Settings.FskPacketHandler.FrequencyDeviation );
                }
                SX1272.Settings.FskPacketHandler.PreambleDetected = false;
                SX1272.Settings.FskPacketHandler.SyncWordDetected = false;
                SX1272.Settings.FskPacketHandler.NbBytes = 0;
            ...
            case MODEM_LORA:
                {
                    ...
                    {
                        // If LoRa detects a CRC fail, the program enters this section of the code.
                        // Tested by removing the RF cable and waiting >2 mins for CRC fail.
                        // (It takes a while because the signal still radiates well without the RF cable.)

                        // PA Consulting added these two lines of code.
                        // Read the FIFO, to remove the message that contains the CRC failure.
                    ...
                    SX1272.Settings.LoRaPacketHandler.FrequencyDeviation = SX1272ReadFrequencyDeviation(MODEM_LORA);
                    ...
                    if( ( RadioEvents != NULL ) && ( RadioEvents->RxDone != NULL ) )
                    {
                        RadioEvents->RxDone( RxTxBuffer, SX1272.Settings.LoRaPacketHandler.Size, 
                       SX1272.Settings.LoRaPacketHandler.RssiValue, SX1272.Settings.LoRaPacketHandler.SnrValue, SX1272.Settings.LoRaPacketHandler.FrequencyDeviation );
                    }
                }
                break;
            default:
...
// Handles the RxTimeout interrupt
void SX1272OnDio1Irq( void )
{
    switch( SX1272.Settings.State )
    {
...
// Handles the LoRa frequency hopping
void SX1272OnDio2Irq( void )
{
    switch( SX1272.Settings.State )
    {
        case RF_RX_RUNNING:
            ...
            case MODEM_FSK:
                if( ( SX1272.Settings.FskPacketHandler.PreambleDetected == true ) && ( SX1272.Settings.FskPacketHandler.SyncWordDetected == false ) )
                {
                    ...
                    SX1272.Settings.FskPacketHandler.FrequencyDeviation = SX1272ReadFrequencyDeviation(MODEM_FSK);
