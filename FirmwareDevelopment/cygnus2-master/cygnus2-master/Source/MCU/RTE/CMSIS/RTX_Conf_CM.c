...
//   <i> Default: 0
 ...
 #define OS_PRIVSTKSIZE 1468       // this stack size value is in words
#endif
...
void os_error (uint32_t error_code) {
   osThreadId last_thread = svcThreadGetId();
  ...
  switch (error_code) {
    ...
    sprintf(reset_message,"ERROR: os_error->OS_ERROR_STACK_OVF: thread = 0x%08X", (uint32_t)last_thread);
      break;
    case OS_ERROR_FIFO_OVF:
      /* ISR FIFO Queue buffer overflow detected. */
    sprintf(reset_message,"ERROR: os_error->OS_ERROR_FIFO_OVF: thread = 0x%08X", (uint32_t)last_thread);

      break;
    case OS_ERROR_MBX_OVF:
      /* Mailbox overflow detected. */
    sprintf(reset_message,"ERROR: os_error->OS_ERROR_MBX_OVF: thread = 0x%08X", (uint32_t)last_thread);

      break;
    case OS_ERROR_TIMER_OVF:
      /* User Timer Callback Queue overflow detected. */
    sprintf(reset_message,"ERROR: os_error->OS_ERROR_TIMER_OVF: thread = 0x%08X", (uint32_t)last_thread);

    break;
    default:
       sprintf(reset_message,"ERROR: os_error->default: thread = 0x%08X", (uint32_t)last_thread);
    break;
  }

  Error_Handler(reset_message);
