                ...
                isRadioActive( false )
{
    ...
    this->lastContinuousDataBit = false;
    this->sendDataBit = false;
    this->enableContinuousDataToggle = false;
    this->sxDataZeroCount = 0;
    this->sxDataOneCount = 0;
    this ->berCount = 0;
    ...
    case MODEM_FSK:
        {
            ...
            this->settings.Fsk.RxPacketMode = rxPacketMode;
...
void SX1272::Tx( uint32_t timeout )
{
    ...
    case MODEM_FSK:
        {
           if ( this->settings.Fsk.RxPacketMode )
           {
               // DIO0=PacketSent
               // DIO1=FifoEmpty
               // DIO2=FifoFull
           ...
           {
               Write( REG_DIOMAPPING1, ( RF_DIOMAPPING1_DIO0_11 | RF_DIOMAPPING1_DIO1_00 | RF_DIOMAPPING1_DIO2_00 | RF_DIOMAPPING1_DIO3_00 ));

               Write( REG_DIOMAPPING2, ( Read( REG_DIOMAPPING2 ) & RF_DIOMAPPING2_DIO4_MASK & RF_DIOMAPPING2_DIO5_MASK &
                                                                      RF_DIOMAPPING2_MAP_MASK ) |
    ...
    switch( this->settings.State )
    {
        case RF_RX_RUNNING:
            ...
            case MODEM_FSK:
                ...
                {
                    this->RadioEvents->RxDone( rxtxBuffer, this->settings.FskPacketHandler.Size, this->settings.FskPacketHandler.RssiValue, 0, 0, 0 );
                }
                this->settings.FskPacketHandler.PreambleDetected = false;
                this->settings.FskPacketHandler.SyncWordDetected = false;
                this->settings.FskPacketHandler.NbBytes = 0;
            ...
            case MODEM_LORA:
                {
                    ...
                    // RM: Reading FEI ------------
                    uint32_t aux_reg = 0;
                    uint32_t local_fei = 0;
                    float myFloat = 0;
                    int32_t my_reg = 0;
                    
                    local_fei = Read( REG_LR_FEIMSB );
                    local_fei <<= 8;
                    
                    aux_reg = Read( REG_LR_FEIMID );
                    local_fei |= aux_reg;
                    local_fei <<= 8;
                    
                    aux_reg = Read( REG_LR_FEILSB );
                    local_fei |= aux_reg;
                    
                    this->settings.LoRaPacketHandler.FeiValue = local_fei;
                    
                    my_reg = (int32_t)(local_fei - 1);
                    my_reg = ~my_reg;
                    
                    if(local_fei > 0x00080000)
                    {
                        my_reg = my_reg & 0x000FFFFF;
                    }
                    
                    myFloat = 16777216;
                    myFloat = myFloat / 32000000;
                    myFloat = myFloat * my_reg; 
                    
                    this->settings.LoRaPacketHandler.Ferror = myFloat;
                    ...
                    if( ( this->RadioEvents != NULL ) && ( this->RadioEvents->RxDone != NULL ) )
                    {
                        this->RadioEvents->RxDone( rxtxBuffer, this->settings.LoRaPacketHandler.Size, this->settings.LoRaPacketHandler.RssiValue, this->settings.LoRaPacketHandler.SnrValue, this->settings.LoRaPacketHandler.FeiValue, this->settings.LoRaPacketHandler.Ferror );
                    }
                }
                break;
            default:
...
void SX1272::OnDio1Irq( void )
{
    switch( this->settings.State )
    {
        case RF_RX_RUNNING:
            ...
            {
            case MODEM_FSK:
                if ( this->settings.Fsk.RxPacketMode == true )
                {
                   // FifoLevel interrupt
                   // Read received packet size
                   if( ( this->settings.FskPacketHandler.Size == 0 ) && ( this->settings.FskPacketHandler.NbBytes == 0 ) )
                ...
                else
                {
                   if ( sequencerEnabled )
                   {
                      //continuous mode.  The clock on DIO1 has triggerred an interupt.
                      //Read the data level on pin DIO2
   //                   sx_data.input();
                      bool previousDataBit = lastContinuousDataBit;
                      lastContinuousDataBit = sx_data;
                      if ( previousDataBit == lastContinuousDataBit )
                      {
                         berCount++;
                      }
                      
                      if ( lastContinuousDataBit == 0 )
                      {
                         sxDataZeroCount++;
   //                      this->dio2.rise(callback(this, &SX1272::OnDio2Irq));
                      }
                      else
                      {
                         sxDataOneCount++;
   //                      this->dio2.fall(callback(this, &SX1272::OnDio2Irq));
                      }
                   }
                }
                break;
        ...
        case RF_TX_RUNNING:
            ...
            case MODEM_FSK:
                ...
                {
                   if ( enableContinuousDataToggle )
                   {
                      lastContinuousDataBit = !lastContinuousDataBit;
                      this->sendDataBit = true;
                   }
                   //continuous mode.  The clock on DIO1 has triggerred an interupt.
                   //Write the data level to pin DIO2
                   if ( this->sendDataBit )
                   {
                      this->sendDataBit = false;
                      if ( sequencerEnabled )
                      {
                         sx_data = lastContinuousDataBit;
                      }
                      else
                      {
                         sx_data.output();
        ...
        case RF_IDLE:
         {
            ...
            if ( this->settings.Fsk.RxPacketMode == false &&
                  this->settings.Fsk.RxContinuous &&
                  this->sendDataBit == true )
            {
                this->sendDataBit = false;
                  sx_data.output();
...
*/
void SX1272::SetContinouousDatabit( bool bitValue )
{
   lastContinuousDataBit = bitValue;
   this->sendDataBit = true;
}
...
*/
bool SX1272::GetContinouousDatabit( const bool buffered )
{
   bool result = false;
   if ( buffered )
   {
      if ( settings.Fsk.RxPacketMode == false )
      {
         result = lastContinuousDataBit;
      }
   }
   else
   {
      uint32_t pinState = sx_data;
      if ( pinState != 0 )
      {
         result = true;
      }
...
*/
void SX1272::EnableContinouousDatabitToggle( bool value )
{
   enableContinuousDataToggle = value;
}
...
*/
bool SX1272::ContinouousDatabitToggleEnabled( void )
{
   return enableContinuousDataToggle;
}
...
*/
int16_t SX1272::GetFskRSSI( void )
{
   int16_t raw_rssi = Read( REG_RSSIVALUE );
   int16_t rssi = -( raw_rssi >> 1) + 22; //22 Db correction determined by measurement.
   return rssi;
}
   ...
   */
   void SX1272::ReadAndResetSXDataCounters( uint32_t* zeroCount, uint32_t* oneCount, double* ber  )
   {
      if( ber )
      {
         if ( (sxDataOneCount + sxDataZeroCount) == 0 )
         {
            *ber = 100.0;
         }
         else
         {
            *ber = 100.0 * ((double)berCount / 2) / (double)(sxDataOneCount + sxDataZeroCount);
         }
         berCount = 0;
      }
      if ( zeroCount )
      {
         *zeroCount = sxDataZeroCount;
         sxDataZeroCount = 0;
      }
      if ( oneCount )
      {
         *oneCount = sxDataOneCount;
         sxDataOneCount = 0;
      }
...
/*!
* @brief Sets the Tx to continuous transmit (pure unmodulated carrier)
*
* @param [IN] freq         Set the Tx Frequency [Hz]
* @param [IN] power        Sets the output power [dBm]
* @param [IN] timeout      Sets the timeout [us]
*/
void SX1272::SetTxCW( uint32_t freq, int8_t power, uint32_t timeout )
{
    SetChannel( freq );

   SetTxConfig( MODEM_FSK, power, 0, 0, 4800, 0, 5, false, false, 0, 0, 0, timeout, false );
...
/*!
* @brief Sets the Tx to FSK continuous mode (data on DIO2 goes straight to modulator )
*
* @param [IN] freq                        Set the Tx Frequency [Hz]
* @param [IN] power                       Sets the output power [dBm]
* @param [IN] frequencyDeviation          Sets the frequency deviation [Hz]
* @param [IN] bandwidth                   Sets the bandwidth [Hz]
* @param [IN] dataRate                    Sets the data rate [bits/s]
* @param [IN] enableSequencer             Produce a clock signal on DIO1
* @param [IN] timeout                     Sets the timeout [ms]
*/
void SX1272::SetTxContinuousMode( uint32_t freq, int8_t power, uint32_t frequencyDeviation, uint32_t bandwidth, uint32_t dataRate, bool enableSequencer, uint32_t timeout )
{
   Sleep();
   SetChannel( freq );
   SetTxConfig( MODEM_FSK, power, frequencyDeviation, bandwidth, dataRate, 0, 12, false, false, false, 0, false, timeout, false );
   
   settings.Fsk.RxContinuous = true;
   
   //Set the 1272 to continuous mode
   Write( REG_PACKETCONFIG1, 0x00 );
   Write( REG_PACKETCONFIG2, RF_PACKETCONFIG2_DATAMODE_CONTINUOUS );
   ...
   if ( enableSequencer )
   {
      ...
      Write( REG_DIOMAPPING2, ( Read( REG_DIOMAPPING2 ) & RF_DIOMAPPING2_DIO4_MASK & RF_DIOMAPPING2_DIO5_MASK &
                                                                      ...
                                                                      RF_DIOMAPPING2_DIO5_10 |
                                                                      RF_DIOMAPPING2_MAP_PREAMBLEDETECT );
      //Configure the sequencer to produce a clock signal on DIO1
      //Write( REG_SEQCONFIG1, ( RF_SEQCONFIG1_FROMSTART_TOTX | RF_SEQCONFIG1_FROMIDLE_TOTX ) );
      Write( REG_SEQCONFIG2, 0x00 );
      //Start the sequencer
      Write( REG_OOKPEAK, ( Read( REG_OOKPEAK ) | RF_OOKPEAK_BITSYNC_ON ) );
      Write( REG_SEQCONFIG1, ( Read( REG_SEQCONFIG1 ) | RF_SEQCONFIG1_SEQUENCER_START ) );
   ...
   SetOpMode( RF_OPMODE_TRANSMITTER );
   sx_data.output();
   this->dio1.rise(callback(this, &SX1272::OnDio1Irq));
}

/*!
* @brief Sets the Rx to FSK continuous mode (Demodulator output on DIO2)
*
* @param [IN] freq                        Set the Tx Frequency [Hz]
* @param [IN] frequencyDeviation          Sets the frequency deviation [Hz]
* @param [IN] bandwidth                   Sets the bandwidth [Hz]
...
* @param [IN] enableSequencer             Produce a clock signal on DIO1
*/
void SX1272::SetRxContinuousMode( uint32_t freq, uint32_t frequencyDeviation, uint32_t bandwidth, uint32_t dataRate, bool use_lora_crc, bool enableSequencer )
{
   Sleep();
   SetChannel( freq );

   SetRxConfig( MODEM_FSK, bandwidth, dataRate, 0, bandwidth, 12, 0, false, 0, use_lora_crc, false, 0, false, true, false );
   ...
   settings.Fsk.RxContinuous = true;
   settings.Fsk.RxPacketMode = false;
   
   if ( enableSequencer )
   {
      Write( REG_DIOMAPPING1, ( RF_DIOMAPPING1_DIO0_11 | RF_DIOMAPPING1_DIO1_00 | RF_DIOMAPPING1_DIO2_00 | RF_DIOMAPPING1_DIO3_10 ));
      ...
      Write( REG_DIOMAPPING2, ( Read( REG_DIOMAPPING2 ) & RF_DIOMAPPING2_DIO4_MASK & RF_DIOMAPPING2_DIO5_MASK &
                                                                      ...
                                                                      RF_DIOMAPPING2_MAP_PREAMBLEDETECT );
      //Configure the sequencer to produce a clock signal on DIO1
      Write( REG_SEQCONFIG2, RF_SEQCONFIG2_FROMRXPKT_TORX );
      //Start the sequencer
      Write( REG_OOKPEAK, ( Read( REG_OOKPEAK ) | RF_OOKPEAK_BITSYNC_ON ) );
      Write( REG_SEQCONFIG1, ( Read( REG_SEQCONFIG1 ) | RF_SEQCONFIG1_SEQUENCER_START ) );
   ...
   {
      // same setup as with synchroniser but no clock output on DIO1
      Write( REG_DIOMAPPING1, ( RF_DIOMAPPING1_DIO0_11 | RF_DIOMAPPING1_DIO1_10 | RF_DIOMAPPING1_DIO2_00 | RF_DIOMAPPING1_DIO3_10 ));
