...
GSMText::GSMText( SMSContact* contact, short	actions ) : contact( contact ), actions( actions ), faultgroup( 0 ), count( 0 )
{
	
}

GSMText::GSMText( SMSContact* contact, short	actions, char faultgroup, char count ) : contact( contact ), actions( actions ), faultgroup( faultgroup ), count( count )
{
...
class TestSocket : public Socket
{
	...
	TestSocket(): Socket( "5.tcp.eu.ngrok.io", "google.co.uk", 19166, PROT_TCP, true )
	{
		this->dest_ip[0] = 52;
		this->dest_ip[1] = 166;
		this->dest_ip[2] = 12;
...
Socket* GSM::socket = &sock;
GSMState GSM::state = GSM_ECHO_OFF;
GSMState GSM::next_state = GSM_SET_MNO_PROFILE;
...
GSM::GSM( ) : Module( "GSM", GSM_PRIORITY, EVENT_CLEAR | EVENT_RESET ), textqueue( (GSMText*) textqueuebuff, GSM_TEXT_QUEUE_SIZE )					
{
	...
	if ( !EEPROM::GetVar( SMS_Enabled, enabled ) )
	{
		if ( !EEPROM::GetVar( SMS_Enabled, enabled ) )
		{
			Fault::AddFault( FAULT_SMS_ENEBLED_GET );
			//Log::Msg(LOG_SYS, "01_GSMTest.cpp -- ADD");
		}
	}
...
void GSM::Reset( )
{
	...
	txready = true;
	state = GSM_ECHO_OFF;
	next_state = GSM_SET_MNO_PROFILE;
...
int GSM::Init( )
{
	...
	*apn  = 0;
	...
	if ( !EEPROM::GetVar( AppSupp_APN, *apn ) )
	{
		if ( !EEPROM::GetVar( AppSupp_APN, *apn ) )
		{
			Fault::AddFault( FAULT_AT_GSM_APN_SET );
			//Log::Msg(LOG_SYS, "02_GSMTest.cpp -- ADD");
		}
	}
	apn[ 39] = 0;
...
//#endif		
	if (1)
	{
		...
		else if ( prev_state == state && state != GSM_IDLE )
		{
			...
			if ( prev_state_count++ > 60 )
			{
				app.DebOut( "GSM - stuck on same thing..\n" );
				prev_state_count = 0;
				if ( state == GSM_CHECK_ATTATCHMENT )
				{
		...
		//	app.gsm_uart->RxXferCount = app.gsm_uart->hdmarx->Instance->NDTR;
			...
			if ( ++second > 20 ) // (not quite seconds)
			{
				second = 0;
				txready = true;
...
void GSM::ProcessRx( int bytes )
{
	...
	{		
		...
		{
			if ( ResultOK() )	{ state = GSM_POWER_UP; next_state = GSM_SEARCH_NETWORK; }
			break;
		}
		...
		case GSM_CHECK_DOMAIN :
		{
			...
			if ( dom != NULL )
			{
				...
				int got = sscanf( dom, "USVCDOMAIN: %d", &network_domain );
		...
		{
			if ( ResultOK() )  state = GSM_CHECK_REGISTRATION; 
			break;
		}
		...
		case GSM_ATTACH_GPRS:
		{
			if ( ResultOK() ) state = GSM_ECHO_OFF;  
			break;
		}
		...
		case GSM_CREATE_SOCKET:
		{
			...
			if ( sscanf( sock, "+USOCR: %d", &socket->num ) == 1 )
			{
				...
				{
					state = GSM_OPEN_SOCKET; //GSM_SOCKET_OPTION;
				}
				else
				{
...
int GSM::Receive( Command* cmd )
{
	switch ( cmd->type )
	{
		case CMD_SET_GSM_SETTINGS:
		{
			MCC = cmd->int0 ;
			MNC = cmd->int1 ;
			PIN = cmd->int2 ;
			
			if (  eeprom->SetVar( GSM_MCC, MCC ) &&
					eeprom->SetVar( GSM_MNC, MNC ) &&
					eeprom->SetVar( GSM_PIN, PIN ) )
			{
				return CMD_OK;
			}
		...
		case CMD_SET_SMS :
		{
			...
			if ( !EEPROM::SetVar( SMS_Enabled, enabled ) )
			{
				if ( !EEPROM::SetVar( SMS_Enabled, enabled ) )
				{
					Fault::AddFault( FAULT_SMS_ENEBLED_SET );
					//Log::Msg(LOG_SYS, "03_GSMTest.cpp -- ADD");
				}
				else
				{
					Fault::RemoveFault( FAULT_SMS_ENEBLED_SET );
					//Log::Msg(LOG_SYS, "04_GSMTest.cpp -- REMOVE");
				}
			}
			else
			{
				Fault::RemoveFault( FAULT_SMS_ENEBLED_SET );
				//Log::Msg(LOG_SYS, "05_GSMTest.cpp -- REMOVE");
			}
