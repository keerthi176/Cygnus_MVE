...
void BIST::ListI2C( I2C_HandleTypeDef* bus )
{
 	for( uint16_t n = 0x10; n < 0xFF; n+=2 )
	{
		if (  HAL_I2C_IsDeviceReady( bus, n, 4, 100) == HAL_OK )
		{
			app->DebOut( "%d ! \n", n );
		}
	}
...
int BIST::CheckEEPROM( I2C_HandleTypeDef* bus, int device )
{
	int pass = 0;
