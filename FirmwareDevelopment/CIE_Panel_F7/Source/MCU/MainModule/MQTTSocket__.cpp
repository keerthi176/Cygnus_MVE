...
void MQTTSocket::Init( time_t expiry, char* device, char* key, void (*reset_cb)( void ) )
{
...
//					 LWIP_CONST_CAST(void*, &mqtt_client_info));
	  ...
	  if ( mqtt_client_connect( mqtt_client,
				 ...
				 &mqtt_client_info) == ERR_OK )
		{
			...
			memcpy( connection, mqtt_client->output.buf + mqtt_client->output.get, connection_len );
...
int MQTTSocket::Connect( void (*callback)( char* data, int length ) )
{
	 if ( connection_len > 0 )
	 {
		 response = callback;
