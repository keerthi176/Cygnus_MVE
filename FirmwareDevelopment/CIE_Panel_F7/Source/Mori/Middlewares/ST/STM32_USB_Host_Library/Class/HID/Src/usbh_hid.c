...
{
	phost->USBH_ClassTypeDef_pData[ phost->device.current_interface ] = (HID_HandleTypeDef *)USBH_malloc(sizeof(HID_HandleTypeDef));
	memset(phost->USBH_ClassTypeDef_pData[ phost->device.current_interface ], 0, sizeof(HID_HandleTypeDef));
	HID_HandleTypeDef *p = (HID_HandleTypeDef *)phost->USBH_ClassTypeDef_pData[ phost->device.current_interface ];
	p->state = HID_ERROR;
	phost->interfaces++;
	return p;
}
...
USBH_UsrLog ("Mouse found.");
			...
			//		HID_Handle->Init =  USBH_HID_MouseInit;
				...
				if( /*( pif->bInterfaceProtocol == HID_MOUSE_BOOT_CODE ||
					pif->bInterfaceProtocol == HID_KEYBRD_BOOT_CODE  ) &&*/
					/*if_ix > 0  &&*/ HID_Handle == NULL)
				{
...
USBH_UsrLog ("Mouse found.");
		...
		case 5:
		 	...
		 	USBH_SelectInterface(phost, 0 );//phost->device.CfgDesc.Itf_Desc[0].bInterfaceNumber);
			state = 0;
			status = USBH_OK;
			break;
	}
...
{   
	...
	HID_HandleTypeDef *HID_Handle = phost->USBH_ClassTypeDef_pData[ phost->device.current_interface ];
...
inline static void USBH_HID_IncrementInterface(USBH_HandleTypeDef *phost)
{
	// switch interface
...
//LOG1(":");
			...
			else USBH_HID_IncrementInterface(phost);
			break;
...
static USBH_StatusTypeDef USBH_HID_SOFProcess(USBH_HandleTypeDef *phost)
{
	...
	{
		if(( phost->Timer - HID_Handle->timer) >= 400 ) //HID_Handle->poll[0])
		{
			HID_Handle->state = HID_GET_DATA;
