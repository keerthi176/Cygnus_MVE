...
static FTDI_HandleTypeDef ftdi_handle = { 0 };
...
//	 FTDI_Handle = phost->USBH_ClassTypeDef_pData[phost->interfaces] =  (FTDI_HandleTypeDef *)USBH_malloc (sizeof(FTDI_HandleTypeDef));
	 FTDI_Handle = phost->USBH_ClassTypeDef_pData[phost->interfaces] = &ftdi_handle;	// only 1 ftdi device sorry. 
	 memset( &ftdi_handle, 0, sizeof( ftdi_handle ) );   
    ...
    else
    {
      ...
      FTDI_Handle->state = FTDI_IDLE_STATE;
		FTDI_Handle->req_stage = REQ_RESET;
...
static USBH_StatusTypeDef USBH_FTDI_InterfaceDeInit (USBH_HandleTypeDef *phost)
{
	...
	{
		FTDI_HandleTypeDef *FTDI_Handle = phost->USBH_ClassTypeDef_pData[idx];
	...
	switch ( FTDI_Handle->req_stage )
	{
		case REQ_RESET :
		{
			status = Reset( phost );
		 	...
		 	if ( status == USBH_OK )
			{
				FTDI_Handle->req_stage = REQ_SET_BAUD;
			}
			else break;
		}
		case REQ_SET_BAUD :
		{
		   ...
		   if( status == USBH_OK )
		   {
			    FTDI_Handle->req_stage = REQ_SET_CONFIG;
		   }
			else break;
		}
		case REQ_SET_CONFIG :
		{
			status = USBH_FTDI_Setup( phost, FTDI_SIO_SET_DATA_STOP_BITS_1, FTDI_SIO_SET_DATA_PARITY_NONE, 8 );
			...
			if( status == USBH_OK )
		   {
			   phost->pUser( phost, HOST_USER_CLASS_ACTIVE );
				...
				// Start receving
 			   USBH_FTDI_Receive( phost, rxdata, 64 ); 
		   }
		}
   }
   return status;
...
static USBH_StatusTypeDef Reset( USBH_HandleTypeDef *phost )
{
   ...
   FTDI_Handle->readbuffer_offset = 0;
   FTDI_Handle->readbuffer_remaining = 0;
...
static USBH_StatusTypeDef SetBaud(USBH_HandleTypeDef *phost, int baudrate )
{
   ...
   unsigned short value, index;
    ...
    if (FTDI_Handle->bitbang_enabled)
    {
        baudrate = baudrate*4;
    }

    actual_baudrate = ftdi_convert_baudrate(baudrate, FTDI_Handle, &value, &index);
...
USBH_StatusTypeDef USBH_FTDI_BreakCtrl( USBH_HandleTypeDef *phost, int break_state )
{
   ...
   FTDI_HandleTypeDef *FTDI_Handle = phost->USBH_ClassTypeDef_pData[ idx];
	...
	if (break_state)
		...
		value = FTDI_Handle->last_set_data_value;

	phost->Control.setup.b.bmRequestType = FTDI_SIO_SET_DATA_REQUEST_TYPE;                                          
	phost->Control.setup.b.bRequest = FTDI_SIO_SET_DATA;  
	phost->Control.setup.b.wValue.w = value;
	phost->Control.setup.b.wIndex.w = FTDI_Handle->interface;
	phost->Control.setup.b.wLength.w = 0;
...
USBH_StatusTypeDef USBH_FTDI_Setup( USBH_HandleTypeDef *phost, uint16_t stopbits, uint16_t parity, uint16_t databits )
{
   ...
   FTDI_HandleTypeDef *FTDI_Handle = phost->USBH_ClassTypeDef_pData[ idx];
	...
	FTDI_Handle->last_set_data_value = value;
...
USBH_StatusTypeDef  USBH_FTDI_Transmit(USBH_HandleTypeDef *phost, const uint8_t *pbuff, uint32_t length)
{
  ...
  {
    FTDI_Handle->pTxData = (uint8_t *) pbuff;
    ...
    if(URB_Status == USBH_URB_DONE)
    {
      ...
      else
      {
        FTDI_Handle->data_rx_state = FTDI_IDLE;
		  ...
		  USBH_FTDI_Receive( phost, rxdata, 64 ); // ask for more
      }
